% This function represents the Deep Learning Method (Backpropagation with Quasi-Hyperbolic Momentum) 
...
function [w1,w2,w3] = jer_QHM_DL_2(w1,w2,w3,input,observed,T)
t = 1; %Initial step
%tol = 1e-09; %Tolerance
init = zeros(T,1);
for j = 1:T
...
for k = 1:N
  ...
  % delta = ((final_output)+(jer_sigmoid(input_of_output_node).*(1.5-final_output))).*error;
   ...
   %delta1 = jer_LeakyRelu_derivative(input_of_hidden_layer1).*error_of_hidden_layer1;
    ...
    initial_vt1 = (beta*initial_vt1)+((1-beta).*dweight_w1);
   
    initial_vt2 = (beta*initial_vt2)+((1-beta).*dweight_w2);
   
    initial_vt3 = (beta*initial_vt3)+((1-beta).*dweight_w3);

    % Calculate bias correction
    scale1 = alpha*(((1-nu).*dweight_w1)+(nu*initial_vt1));    
    scale2 = alpha*(((1-nu).*dweight_w2)+(nu*initial_vt2));
    scale3 = alpha*(((1-nu).*dweight_w3)+(nu*initial_vt3));
