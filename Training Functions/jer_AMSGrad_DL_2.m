% This function represents the Deep Learning Method (Backpropagation with AMSGrad) 
...
function [w1,w2,w3] = jer_AMSGrad_DL_2(w1,w2,w3,input,observed,T)
t = 1; %Initial step
%tol = 1e-09; %Tolerance
init = zeros(T,1);
for j = 1:T
...
v_hat1 = zeros(hidden,inputs);
v_hat2 = zeros(hidden,hidden);
v_hat3 = zeros(outputs,hidden);
...
for k = 1:N
   ...
   %delta1 = jer_LeakyRelu_derivative(input_of_hidden_layer1).*error_of_hidden_layer1;
    ...
    % Calculate initial correction
    v_hat1 = max(v_hat1,initial_vt1);     
    v_hat2 = max(v_hat2,initial_vt2);   
    v_hat3 = max(v_hat3,initial_vt3);   
  
    scale1 = alpha*(initial_mt1./(sqrt(v_hat1)+epsilon));   
    scale2 = alpha*(initial_mt2./(sqrt(v_hat2)+epsilon));  
    scale3 = alpha*(initial_mt3./(sqrt(v_hat3)+epsilon));   
