% This function represents the Deep Learning Method (Backpropagation with DemonAdam (Decayin Momentum with Adaptive Momentum)) 
...
function [w1,w2,w3] = jer_DemonAdam_DL_2(w1,w2,w3,input,observed,T)
t = 1; %Initial step
tol = 1e-09; %Tolerance
init = zeros(T,1);
for j = 1:T
global inputs hidden outputs;
...
for k = 1:N
   ...
   % delta1 = jer_LeakyRelu_derivative(input_of_hidden_layer1).*error_of_hidden_layer1;
    ...
    initial_mt1 = (beta_t*initial_mt1)+((1-beta_t).*dweight_w1);
    initial_vt1 = (beta_2*initial_vt1)+((1-beta_2).*dweight_w1.^2);

    initial_mt2 = (beta_t*initial_mt2)+((1-beta_t).*dweight_w2);
    initial_vt2 = (beta_2*initial_vt2)+((1-beta_2).*dweight_w2.^2);

    initial_mt3 = (beta_t*initial_mt3)+((1-beta_t).*dweight_w3);
    initial_vt3 = (beta_2*initial_vt3)+((1-beta_2).*dweight_w3.^2);

    % Calculate bias correction
    m_hat1 = initial_mt1./(1-beta_t^t);
    v_hat1 = initial_vt1./(1-beta_2^t);

    m_hat2 = initial_mt2./(1-beta_t^t);
    v_hat2 = initial_vt2./(1-beta_2^t);

    m_hat3 = initial_mt3./(1-beta_t^t);
